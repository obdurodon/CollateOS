...
for f in filter(lambda x: str(x.split('.')[len(x.split('.'))-1]) == 'xml' , os.listdir(path)):
    html = codecs.open(f+ '.html', 'w')
    html.write('<html><head><title>' + f + '</title></head><body><h2>' + f + '</h2><table border="1"><th>Original</th><th>Conflated</th>')
    ...
    for i in list(splitTagsFromText(xmlDoc)):
        if not i.startswith('<'):
            for w in i.split():
                html.write('<tr><td>' + w.encode('utf-8') + '</td><td>' + conflate(w).encode('utf-8') + '</td></tr>')
